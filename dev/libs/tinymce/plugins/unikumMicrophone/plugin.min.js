...
function runViewLogic(editor) {
	stageController = {
	  ...
	  stages: [
        ...
        run: function() {
          $('.umm_convert_progress span').text(0+'%');

					$('.umm_stage6_cancel_recording').on('click', function(){
						stageController.transitionStage(0);
					});
					stageController.elementsToUnbind.push(
						'.umm_stage6_cancel_recording'
					);
				}
	    },
...

      {
        ...
        wrapperEl: $('#umm_stage7'),
        run: function() {
          $('.umm_stage7_cancel_recording').on('click', function(){
            stageController.transitionStage(0);
          });
          stageController.elementsToUnbind.push(
            '.umm_stage7_cancel_recording'
          );
        }
      }
	  ],
	  ...
	  transitionStage: function(nStage, event) {
	    // Change view/stage
      $('.umm_file_tolarge').removeClass('show');
	...
	uploadFile: function() {
      ...
      if(message !== 'upload') {
        $('.umm_stage0_rec_btn_upload').removeClass('umm_rec_btn_uploading');
			...
			if (message == 'error' || message == 'cantconvert') {
        stageController.transitionStage(7);
      ...
      if (message == 'converting-big') {
        console.log('biiiig', $('.umm_file_tolarge'))
        stageController.transitionStage(6);
        $('.umm_file_tolarge').addClass('show');
      }
        ...
        PreviewPlayer.setName(file.name.split('.')[0]);
			};
